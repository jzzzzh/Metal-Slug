...
void putToggleImagePng(int img_x, int img_y, IMAGE *pSrcImg)
{
    ...
    for (int i = 0; i < im_height; i++)
    {
        ...
        {
            int src_r = ((src[im_width - j - 1] & 0xff0000) >> 16);
            int src_g = ((src[im_width - j - 1] & 0xff00) >> 8);
            int src_b = src[im_width - j - 1] & 0xff;
            int src_a = ((src[im_width - j - 1] & 0xff000000) >> 24);
